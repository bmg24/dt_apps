  ...
  return (
    <>
      ...
      <Flex flexDirection="column" padding={32}>
        <DQLEditor value={queryString} onChange={(event) => setQueryString(event)} />
        <Flex justifyContent={error ? "space-between" : "flex-end"}>
          ...
          {queryState === "success" && !result?.records && <Paragraph>no data available</Paragraph>}
          {queryState === "success" && result?.records && !visualRecommendations.includes("TimeSeriesChart") && (
